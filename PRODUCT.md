# 弹幕漂移产品说明

## 产品目标

做一个短局、低启动成本的个人网页小游戏。玩家通过移动主角球躲避弹幕，在存活时间中获得分数，并通过拾取道具制造喘息和反转机会。

## 产品类型

浏览器游戏 / 互动玩具。当前实现是静态网页，不依赖后端。

## 核心循环

1. 玩家开始一局。
2. 敌对球从场边进入，数量、速度和模式逐步增强。
3. 玩家移动躲避，分数随存活时间增长。
4. 场内偶尔出现道具球，玩家可选择冒险拾取。
5. 碰撞失败后显示结算，可立刻重新开始。

## 当前范围

- Canvas 2D 弹幕躲避玩法。
- 键盘、指针和触控移动；触控采用相对拖动，降低手指遮挡主角的问题。
- 分数、存活时间、压力、最高分和道具状态 HUD。
- 失败重开。
- 失败结算展示本局得分、存活时间和最高分。
- 暂停和继续，恢复后保持同一局的主角位置、弹幕布局、分数和时间。
- 桌面端支持全屏游玩，全屏内隐藏标题、侧栏和 HUD，只保留游戏场地、暂停按钮和退出全屏按钮。
- 手机端横屏默认进入全屏游戏布局，竖屏显示规则说明和横屏提示。
- 中文 / 英文切换，语言偏好保存在 `localStorage`。
- 难度梯度：少量慢速蓝色小球，蓝色大球，红色单球，红色弧形弹幕。
- 弹幕节奏分为热身、压迫、混合、爆发四段。
- 道具球：清场、护盾、减速。
- 时效道具使用跟随主角的数字倒计时反馈。
- 侧栏说明道具符号，避免玩家猜测。
- 轻量音效和移动端震动反馈。
- 弹幕与粒子数量有性能上限。

## 产品判断

“短冲刺”暂不进入当前版本。它能增加主动操作，但会把游戏重心推向动作技巧，并增加移动端操作设计成本。

当前选择道具球系统，因为它更直接改变弹幕局势：

- 清场：提供即时释放压力的强反馈。
- 护盾：允许玩家短时间穿越危险区域。
- 减速：延长决策窗口，提升策略感。

这三种道具覆盖了即时、保护、节奏控制三类效果，学习成本低，适合作为第一批道具。

## 候选道具池

后续可以评估，但不应一次性加入：

- 分数倍率：提高风险收益，但可能鼓励拖延刷分。
- 磁吸道具：降低拾取成本，但可能削弱走位意义。
- 时间冻结：强反馈明确，但和减速效果接近。
- 一次性护盾：比时限护盾更容易平衡。
- 高风险诅咒道具：适合后期加入，不适合当前 MVP。

## 验收标准

- 打开 `index.html` 或本地服务器页面后第一屏可直接开始游戏。
- 开始后敌对球会生成并进入场地。
- 玩家移动对键盘、指针和触控输入有明显响应。
- 触控相对拖动时，手指不需要按在主角球上也能移动。
- 存活时分数增长，失败后停止增长。
- 碰撞敌对球会进入失败态。
- 失败后可以不刷新页面直接重开。
- 失败结算能让玩家看清本局表现和最高分变化。
- 最高分保存在 `localStorage`。
- 语言切换会更新可见 UI 文案和辅助标签，并保存在 `localStorage`。
- 暂停时游戏状态冻结，继续后恢复同一局；`Esc` 只用于退出桌面全屏，不用于暂停。
- 桌面全屏和手机横屏不会显示标题、侧栏或底部 HUD，并且页面不能出现上下滚动。
- 手机竖屏说明页能在单屏内显示完整信息，并明确提示横屏游玩。
- 道具球可拾取并触发对应效果。
- 时效道具倒计时跟随主角显示，不引发 HUD 布局抖动。
- 弹幕阶段会随时间从热身推进到爆发，并改变弹幕类型组合。
- 长时间运行时弹幕和粒子不会无限增长。

## 设备支持边界

- 目标设备：桌面浏览器、普通手机横屏/竖屏、iPad Pro 等大屏触控设备。
- Nest Hub 和 Nest Hub Max 不作为目标设备，仅作为大屏横屏预览处理。
- Nest Hub 系列不保证竖屏说明页、完整触控操作体验或设备旋转行为。

## 暂不做

- 账号、在线排行榜、服务器存档。
- 关卡编辑器、剧情模式、商店或成长系统。
- 复杂移动端手势技能。
